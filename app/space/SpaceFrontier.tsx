...
import { revealResult } from "../interactionNavigation";
...
export default function SpaceFrontier() {
  ...
  const missionDrilldownRef = useRef<HTMLDivElement>(null);
  const missionRecordRef = useRef<HTMLDivElement>(null);
  const delayInstrumentRef = useRef<HTMLDivElement>(null);
  ...
  function resetMissionView() {
    ...
    setHoverYear(null);
    setHoverActor(null);
    revealResult(missionDrilldownRef, "start");
  }
  ...
  return (
    <main className="space-page">
        ...
        <div className="wave-chart" aria-label="Mission records by year">
          {missionYears.map((year) => {
            ...
            return <button
              ...
              onClick={() => { setSelectedYear(year); setSelectedType("All types"); revealResult(missionDrilldownRef, "start"); }}
        ...
        <div className="actor-field">
          ...
          <div className="actor-bars">{actorGroups.map((actor) => {
            ...
            return <button
              ...
              onClick={() => { setSelectedActor(actor); setSelectedType("All types"); revealResult(missionDrilldownRef, "start"); }}
            ><span>{actor}</span><i><b style={{ width: `${actor === "All actors" ? 100 : count / maxActor * 100}%` }} /></i><strong>{count}</strong></button>;
          })}</div>
        </div>

        <div className="mission-drilldown" ref={missionDrilldownRef}>
          ...
          <div className="type-profile">
            ...
            <div className="type-chips"><button type="button" className={selectedType === "All types" ? "selected" : ""} onClick={() => { setSelectedType("All types"); revealResult(missionRecordRef, "start"); }}>All types <span>{selectionBase.length}</span></button>{availableTypes.map(([type, count]) => <button type="button" key={type} className={selectedType === type ? "selected" : ""} onClick={() => { setSelectedType(type); revealResult(missionRecordRef, "start"); }}>{type} <span>{count}</span></button>)}</div>
          </div>
        </div>

        <div className="mission-record-panel" ref={missionRecordRef} aria-live="polite">
      ...
      <section className="autonomy-lab" id="autonomy">
        ...
        <div className="destination-controls" role="group" aria-label="Choose a destination">{destinations.map((item) => <button type="button" key={item.id} className={destination === item.id ? "selected" : ""} onClick={() => { setDestination(item.id); revealResult(delayInstrumentRef, "start"); }}><span>{item.label}</span><small>{item.delay}</small></button>)}</div>
        <div className="delay-instrument" ref={delayInstrumentRef} aria-live="polite">
      ...
      <section className="field-record" id="field-record">
        ...
        <div className="record-tools"><div className="record-filters">{(["All", "Verified", "Changed", "Watch"] as const).map((item) => <button type="button" key={item} className={status === item ? "selected" : ""} onClick={() => { setStatus(item); revealResult(updateListRef, "start"); }}>{item}</button>)}</div><label><span className="sr-only">Search mission updates</span><input value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") revealResult(updateListRef, "start"); }} placeholder="Search mission or agency" /></label></div>
        <div className="update-list" ref={updateListRef} aria-live="polite">{filteredUpdates.map((item) => <article key={item.mission}><div><span className={`record-state state-${item.state.toLowerCase()}`}>{item.state}</span><small>{item.actor}</small></div><h3>{item.mission}</h3><div className="record-change"><p><span>Workbook</span>{item.workbook}</p><b>→</b><p><span>Current field</span>{item.current}</p></div><p className="record-consequence">{item.consequence}</p><a href={item.source} aria-label={`Open official source for ${item.mission}; use Back to return to Mehan Observatory`}>Official source</a></article>)}</div>
